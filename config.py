...
# Юзернейм бота (без @) — нужен для сборки персональных реферальных ссылок
# вида t.me/<BOT_USERNAME>?start=<telegram_id>. Не берём автоматически через
# getMe(), чтобы не дёргать сеть каждый раз, когда кто-то открывает раздел
# "Пригласить" — юзернейм и так не меняется в рамках жизни бота.
