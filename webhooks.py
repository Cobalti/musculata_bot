...
Этот файл — Flask-приложение (реальный запуск через wsgi.py, gunicorn,
см. musculata-webhooks.service). Здесь же (маршрут /telegram-webhook)
теперь ещё и принимаются апдейты от самого Telegram (webhook-режим
вместо polling) — см. wsgi.py, там объяснена вся логика этого перехода.
X_WP_SECRET (webhooks сайта) и TELEGRAM_WEBHOOK_SECRET (webhook
Telegram) — два РАЗНЫХ секрета, не путать.
...
def _handle_referral_conversion(invitee_id: int):
    ...
    из ТЗ по подпискам (п. 3.4): +1/+3/+6 месяцев членства, на 3-й и
    ...
    if result["milestone_reached"]:
        ...
        if extra_days:
            ...
            if new_expires:
                ...
                text += f"\nПрисяга продлена на {months} мес. — теперь действует до {new_expires[:10]}."
            else:
                # У пригласившего сейчас нет активного членства — продлевать
                # нечего (extend_subscription сам это проверяет и логирует).
                # Не обманываем человека сообщением о продлении, если оно
                # не произошло.
                text += "\nБонус на продление учтён, но у тебя сейчас нет активного членства."
...
def payment_success():
    ...
    оплату обычного заказа от оплаты членства (устаревшая ветка — платёж
    теперь на нашей стороне через ЮKassa, см. payments.py; оставлено на
    случай, если Фёдор всё же пришлёт такой вебхук в переходный период),
    поле "type" в теле запроса: "subscription" — активируем членство,
    ...
    if payment_type == "subscription":
        ...
        logger.info("payment-success (членство): telegram_id=%s order_id=%s tier=%s",
