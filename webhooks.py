...
def _handle_referral_conversion(invitee_id: int):
    ...
    из ТЗ по подпискам (п. 3.4): +1/+3/+6 месяцев присяги, на 3-й и
    ...
    if result["milestone_reached"]:
        ...
        if extra_days:
            ...
            if new_expires:
                ...
                # У пригласившего сейчас нет активной присяги — продлевать
                # нечего (extend_subscription сам это проверяет и логирует).
                # Не обманываем человека сообщением о продлении, если оно
                # не произошло.
                text += "\nБонус на продление учтён, но у тебя сейчас нет активной присяги."
...
def payment_success():
    ...
    оплату обычного заказа от оплаты присяги Ордену, ожидаем опциональное
    поле "type" в теле запроса: "subscription" — активируем присягу,
