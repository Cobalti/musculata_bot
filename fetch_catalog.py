...
и раскладывает по трём категориям через поле "role":
    ...
    role: "tier" -> уровень членства в Сообществе
...
Пишет три файла в текущую директорию:
    ...
    real_tiers.json       -- сырые данные об уровнях членства (role=tier) —
                            НЕ подключаем в код, пока не пройдёт созвон
                            с Фёдором по членству; просто сохраняем для
                            справки на будущее
...
def main():
    ...
    print(f"   real_tiers.json    — {len(tiers)} уровней членства (пока не подключаем в код)")
    print(f"\nПаки, которые нашлись:")
    for p in packs:
        print(f"   id={p['id']} | {p['name']} | {p['price']} ₽")
    print(f"\nУровни членства, которые нашлись:")
    for t in tiers:
