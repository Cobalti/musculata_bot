...
# num_threads=2 — дефолт библиотеки, слишком мало: при нескольких почти
# одновременных нажатиях (например, быстро листаешь карточки уровней при
# тестировании, или несколько живых пользователей одновременно) запросы
# встают в очередь на свободный поток, и пока ждут — Telegram успевает
# счесть колбэк "протухшим" ("query is too old"), даже если сам код
# обработчика уже отвечает мгновенно. 16 — с хорошим запасом на несколько
# одновременных тестировщиков; это чисто потоки ожидания сети, не нагрузка
# на CPU, так что цена увеличения минимальна.
bot = telebot.TeleBot(BOT_TOKEN, num_threads=16)
