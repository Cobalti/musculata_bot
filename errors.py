    ...
    def decorator(func):
        ...
        def wrapper(update, *args, **kwargs):
            ...
            try:
                ...
                # Раньше пользователь при ошибке не получал НИЧЕГО — только
                # админ узнавал в личку. Со стороны это выглядело как "бот
                # молчит/ничего не происходит" — особенно опасно для /start,
                # где именно так терялось приветственное сообщение с
                # согласием на ОПД. Теперь юзер тоже получает понятный
                # ответ вместо тишины.
                _reply(
                    bot, update,
